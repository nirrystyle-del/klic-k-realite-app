...
const nameInput=document.getElementById("nameInput"),dayInput=document.getElementById("dayInput"),monthInput=document.getElementById("monthInput"),yearInput=document.getElementById("yearInput"),profileMessage=document.getElementById("profileMessage"),telegramStatus=document.getElementById("telegramStatus"),birthLockInfo=document.getElementById("birthLockInfo"),editBirthdateBtn=document.getElementById("editBirthdateBtn");
...
const PROFILE_KEY=`klic_k_realite_profile_v6_${userKey}`,ACCESS_KEY=`klic_k_realite_test_access_v6_${userKey}`;
...
function renderSubscription(){const source=telegramUser?`Účet: ${telegramUser.id}`:"testovací režim bez Telegram účtu";subscriptionStatus.textContent=hasActiveAccess()?`Testovací režim je aktivní. Budoucí platba se bude ověřovat podle účtu. ${source}.`:`Testovací režim není aktivní. Bez přístupu jsou denní výklady, měsíc a průvodce zamčené. ${source}.`}
...
function renderBirthdateLock(){const p=readProfile();if(!p){setBirthdateFieldsLocked(false);editBirthdateBtn.classList.add("hidden");birthLockInfo.textContent="Příklad: 27 / 6 / 1997";return}
...
const p={name:nameInput.value.trim(),day:newBirth.day,month:newBirth.month,year:newBirth.year,birthdateChangedAt:birthChanged||!old?new Date().toISOString():old.birthdateChangedAt,telegramUser:telegramUser?{id:telegramUser.id,username:telegramUser.username||"",first_name:telegramUser.first_name||""}:null,savedAt:new Date().toISOString()};
saveProfile(p);birthdateEditUnlocked=false;profileMessage.textContent="Profil uložen. Datum narození je nyní uzamčené.";renderBirthdateLock();renderForecast();showScreen(hasActiveAccess()?"today":"access")});
editBirthdateBtn.addEventListener("click",()=>{const p=readProfile();if(!p)return;if(!canChangeBirthdate(p)){profileMessage.textContent=`Datum narození můžete znovu upravit přibližně za ${daysUntilChange(p)} dnů.`;renderBirthdateLock();return}birthdateEditUnlocked=true;profileMessage.textContent="Můžete upravit datum narození. Po uložení se znovu uzamkne.";renderBirthdateLock()});
...
const saved=readProfile();if(saved){nameInput.value=saved.name||"";dayInput.value=saved.day||"";monthInput.value=saved.month||"";yearInput.value=saved.year||""}
renderBirthdateLock();renderForecast();renderSubscription();if(!saved)showScreen("profile");else if(!hasActiveAccess())showScreen("access");else showScreen("today");
